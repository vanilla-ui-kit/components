# VanillaComponents Core (`VC`) — Design Spec (2026-07-03)
...
```
core/core.js            → global `VC` / `VanillaComponents` (the convergence layer)
...
dist/vanilla-components.js → core + all components in one CDN file
```
...
`dist/vanilla-components.js` inside a wrapper that neutralizes each file's
AMD/CJS detection, so in a browser every part attaches its global, and in
Node the bundle exports `{ VC, DatePicker, Toast }`. No minification — the
...
| Channel | How |
|---|---|
| CDN, whole family | `<script src="…/dist/vanilla-components.js">` (one file: core + all atoms) |
| CDN, one atom | `<script src="…/datepicker/datepicker.js">` (unchanged single-file story) |
| npm | `npm i vanilla-components` → `require('vanilla-components')` for `{VC, DatePicker, Toast}`, or deep imports `vanilla-components/datepicker`, `vanilla-components/toast`, `vanilla-components/core` |
...
3. **Own-the-stylesheet middle path** — the raw CSS is exposed as a static
   ...
   (`dist/datepicker.css`, `dist/toast.css`, `dist/vanilla-components.css`),
